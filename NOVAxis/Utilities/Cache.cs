...
namespace NOVAxis.Utilities
{
    ...
    /// <summary>
    /// What a link turned out to be, formats and all, kept under its address. Two requests
    /// for one download - the format list, then the download itself - would otherwise each
    /// pay for an extraction, and what a link resolves to does not change between them.
    ///
    ...
    /// </summary>
    public class AudioMediaCache : Cache<string, AudioTrack>
    {
        public AudioMediaCache(IMemoryCache cache, IOptions<CacheOptions> options)
            : base(nameof(AudioMediaCache), cache, options) { }
    }
