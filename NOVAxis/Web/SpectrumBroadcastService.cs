...
namespace NOVAxis.Web
{
    ...
    public class SpectrumBroadcastService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            if (SpectrumAnalyzer.FftSize < hop)
            {
                ...
                Logger.Warning($"A {SpectrumAnalyzer.FftSize} point window cannot keep up with {rate} " +
                               $"frames a second; {hop} points pass between them and are not analysed");
            }
